...
namespace RinkuApp.Areas.Nominas.Controllers
{
    ...
    public class SalarioController : Controller
    {
        ...
        [Route("Invoice")]
        public IActionResult Invoice(long id, string mes)
        {  
            var data = _service.GetDetails(id, mes);  
            return this.Json(new { record = data });
        }


        [Route("BuscarPorMes")]
        public IActionResult BuscarPorMes(string mes)
        {
            this.ViewBag.Data = _service.GetByMes(mes);
            this.ViewBag.Combo = _service.GetCombo();
